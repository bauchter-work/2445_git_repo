...
x = 0
deviceList = ["0","1","2","3"]  #ADC pins
deviceDescriptions = ["ADC_AIN0","ADC_AIN1","ADC_AIN2","ADC_AIN3"]
pinGain = [ 1024, 1024 , 1024, 1024 ]
pinSPS = [ 8, 8, 8, 8]
...
#Initialize ADC
adc = ADS1x15(ic=ADS1115,address=0x4B,busnum=2)
...
#Header of STDIO:
outputHeader = "UTC Time, RecNum, Description, Voltage, Temp(C), Temp(F)"
...
while True:
	...
	#print "Voltage is: %.8f" % (Voltage)," volts"
        ...
        print strftime("\"%Y-%m-%d %H:%M:%S\"",gmtime())+","+str(recNum)+","+deviceDescriptions[x]+","+str(Voltage)+","+str((Voltage-0.5)/0.005)+","+str(360*(Voltage-0.5)+32)
        if x == 0:
