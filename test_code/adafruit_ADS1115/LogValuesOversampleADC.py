...
# 2014-07-11 - BDA - Added oversample ADC method
...
pinGain = [ 4096, 256 ]
pinSPS = [ 860 , 860 ] #860 is fastest, 8 is slowest
oversampleCount = 100
...
# oversampleADC calls the single-ended sample ADC command 'x' amount of times, as specified by the caller
# and returns a single average value for the single-ended sample, acting as a crude noise reduction filter
def oversampleADC(pin=0,pinGain=4096,pinSPS=8,samples=1):
    pin = int(pin)
    pinGain = int(pinGain)
    pinSPS = int(pinSPS)
    samples = int(samples)
    ...
    for i in range(0,samples,1):
        sampleValue = adc.readADCSingleEnded(pin,pinGain,pinSPS)
	value = value + sampleValue
	#print "Sample Value is: ",sampleValue
...
while True:
    ...
    for x in range(len(deviceList)):
        ...
        Value = oversampleADC(int(deviceList[x]),pinGain[x],pinSPS[x],oversampleCount)
	Voltage = Value / 1000
